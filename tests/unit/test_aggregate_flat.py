...
def test_exposed_instance_reports_raw_named_port_no_verdict() -> None:
    ...
    result = build_flat_records(
        ...
        autonomous_database=_autonomous_database(), identity=_identity(), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(),
...
def test_instance_with_no_vnic_reports_null_facts_not_a_verdict() -> None:
    ...
    result = build_flat_records(
        ...
        networking=_networking(), autonomous_database=_autonomous_database(), identity=_identity(), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(),
        completed_at=COMPLETED_AT,
    )
...
def test_autonomous_database_reports_raw_kms_and_endpoint_facts() -> None:
    ...
    result = build_flat_records(
        ...
        networking=_networking(), autonomous_database=_autonomous_database([adb]), identity=_identity(), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(),
        completed_at=COMPLETED_AT,
    )
...
def test_iam_user_reports_raw_mfa_fact_not_a_verdict() -> None:
    ...
    result = build_flat_records(
        ...
        identity=_identity([user]), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(), completed_at=COMPLETED_AT,
    )
...
def test_deleted_users_and_their_api_keys_are_excluded() -> None:
    ...
    result = build_flat_records(
        ...
        ), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(),
        completed_at=COMPLETED_AT,
    )
...
def test_api_key_reports_raw_creation_timestamp_for_rotation_age_checks() -> None:
    ...
    result = build_flat_records(
        decisions=DECISIONS, discovery=_discovery(), compute=_compute([]),
        networking=_networking(), autonomous_database=_autonomous_database(),
        identity=_identity([user], api_keys_by_user_id={"u1": [api_key]}), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(),
        completed_at=COMPLETED_AT,
    )
...
def test_iam_policy_reports_raw_statements() -> None:
    ...
    result = build_flat_records(
        decisions=DECISIONS, discovery=_discovery(), compute=_compute([]),
        networking=_networking(), autonomous_database=_autonomous_database(),
        identity=_identity(policies=[policy]), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(), completed_at=COMPLETED_AT,
    )
...
def test_identity_disabled_by_default_yields_no_identity_records() -> None:
    ...
    result = build_flat_records(
        decisions=DECISIONS, discovery=_discovery(), compute=_compute([]),
        networking=_networking(), autonomous_database=_autonomous_database(),
        identity=_identity(), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(), completed_at=COMPLETED_AT,
    )
    assert result.records == []
...
def test_bucket_reports_raw_public_access_and_kms_facts() -> None:
    ...
    result = build_flat_records(
        decisions=DECISIONS, discovery=_discovery(), compute=_compute([]),
        networking=_networking(), autonomous_database=_autonomous_database(),
        identity=_identity(), object_storage=_object_storage([bucket]), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(),
        completed_at=COMPLETED_AT,
    )
...
def test_cloud_guard_reports_raw_status_using_tenancy_id() -> None:
    ...
    result = build_flat_records(
        ...
        cloud_guard=_cloud_guard(configuration), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(),
        completed_at=COMPLETED_AT,
    )
...
def test_cloud_guard_disabled_by_default_yields_no_record() -> None:
    result = build_flat_records(
        ...
        networking=_networking(), autonomous_database=_autonomous_database(),
        identity=_identity(), object_storage=_object_storage(),
        cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(),
        completed_at=COMPLETED_AT,
    )
    assert result.records == []
...
def test_alarm_reports_raw_enabled_and_query_facts() -> None:
    ...
    result = build_flat_records(
        ...
        networking=_networking(), autonomous_database=_autonomous_database(),
        identity=_identity(), object_storage=_object_storage(),
        cloud_guard=_cloud_guard(), monitoring=_monitoring([alarm]), load_balancer=_load_balancer(), waf=_waf(),
        completed_at=COMPLETED_AT,
    )
...
def test_load_balancer_reports_raw_is_private() -> None:
    ...
    result = build_flat_records(
        ...
        load_balancer=_load_balancer([lb]), waf=_waf(),
        completed_at=COMPLETED_AT,
    )
...
def test_waf_reports_raw_load_balancer_join() -> None:
    ...
    result = build_flat_records(
        ...
        identity=_identity(), object_storage=_object_storage(),
        cloud_guard=_cloud_guard(), monitoring=_monitoring(),
        load_balancer=_load_balancer(), waf=_waf([web_app_firewall]),
        completed_at=COMPLETED_AT,
    )

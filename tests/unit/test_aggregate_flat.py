...
def _waf(web_app_firewalls=()):
    return WafCollectionResult(web_app_firewalls=list(web_app_firewalls), operations=[])


def _kms_vault(vaults=(), keys=()):
    return KmsVaultCollectionResult(vaults=list(vaults), keys=list(keys), operations=[])
...
def test_exposed_instance_reports_raw_named_port_no_verdict() -> None:
    ...
    result = build_flat_records(
        ...
        autonomous_database=_autonomous_database(), identity=_identity(), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(), kms_vault=_kms_vault(),
...
def test_instance_with_no_vnic_reports_null_facts_not_a_verdict() -> None:
    ...
    result = build_flat_records(
        ...
        networking=_networking(), autonomous_database=_autonomous_database(), identity=_identity(), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(), kms_vault=_kms_vault(),
        completed_at=COMPLETED_AT,
    )
...
def test_domain_complete_and_discovery_complete_pass_through() -> None:
    ...
    assert result.domain_complete == {
        ...
        "loadBalancer": True, "waf": True, "kmsVault": True,
    }
...
def test_autonomous_database_reports_raw_kms_and_endpoint_facts() -> None:
    ...
    result = build_flat_records(
        ...
        networking=_networking(), autonomous_database=_autonomous_database([adb]), identity=_identity(), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(), kms_vault=_kms_vault(),
        completed_at=COMPLETED_AT,
    )
...
def test_iam_user_reports_raw_mfa_fact_not_a_verdict() -> None:
    ...
    result = build_flat_records(
        ...
        identity=_identity([user]), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(), kms_vault=_kms_vault(), completed_at=COMPLETED_AT,
    )
...
def test_deleted_users_and_their_api_keys_are_excluded() -> None:
    ...
    result = build_flat_records(
        ...
        ), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(), kms_vault=_kms_vault(),
        completed_at=COMPLETED_AT,
    )
...
def test_api_key_reports_raw_creation_timestamp_for_rotation_age_checks() -> None:
    ...
    result = build_flat_records(
        decisions=DECISIONS, discovery=_discovery(), compute=_compute([]),
        networking=_networking(), autonomous_database=_autonomous_database(),
        identity=_identity([user], api_keys_by_user_id={"u1": [api_key]}), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(), kms_vault=_kms_vault(),
        completed_at=COMPLETED_AT,
    )
...
def test_iam_policy_reports_raw_statements() -> None:
    ...
    result = build_flat_records(
        decisions=DECISIONS, discovery=_discovery(), compute=_compute([]),
        networking=_networking(), autonomous_database=_autonomous_database(),
        identity=_identity(policies=[policy]), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(), kms_vault=_kms_vault(), completed_at=COMPLETED_AT,
    )
...
def test_identity_disabled_by_default_yields_no_identity_records() -> None:
    ...
    result = build_flat_records(
        decisions=DECISIONS, discovery=_discovery(), compute=_compute([]),
        networking=_networking(), autonomous_database=_autonomous_database(),
        identity=_identity(), object_storage=_object_storage(), cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(), kms_vault=_kms_vault(), completed_at=COMPLETED_AT,
    )
    assert result.records == []
...
def test_cloud_guard_reports_raw_status_using_tenancy_id() -> None:
    ...
    result = build_flat_records(
        ...
        cloud_guard=_cloud_guard(configuration), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(), kms_vault=_kms_vault(),
        completed_at=COMPLETED_AT,
    )
...
def test_cloud_guard_disabled_by_default_yields_no_record() -> None:
    result = build_flat_records(
        ...
        networking=_networking(), autonomous_database=_autonomous_database(),
        identity=_identity(), object_storage=_object_storage(),
        cloud_guard=_cloud_guard(), monitoring=_monitoring(), load_balancer=_load_balancer(), waf=_waf(), kms_vault=_kms_vault(),
        completed_at=COMPLETED_AT,
    )
    assert result.records == []
...
def test_alarm_reports_raw_enabled_and_query_facts() -> None:
    ...
    result = build_flat_records(
        ...
        networking=_networking(), autonomous_database=_autonomous_database(),
        identity=_identity(), object_storage=_object_storage(),
        cloud_guard=_cloud_guard(), monitoring=_monitoring([alarm]), load_balancer=_load_balancer(), waf=_waf(), kms_vault=_kms_vault(),
        completed_at=COMPLETED_AT,
    )
...
def test_load_balancer_reports_raw_is_private() -> None:
    ...
    result = build_flat_records(
        ...
        load_balancer=_load_balancer([lb]), waf=_waf(), kms_vault=_kms_vault(),
        completed_at=COMPLETED_AT,
    )
...
# -- WAF -- existence joined to a load balancer, no policy rule detail --


def test_waf_reports_raw_load_balancer_join() -> None:
    web_app_firewall = _stamp(
        oci.waf.models.WebAppFirewallLoadBalancerSummary(
            id="ocid1.webappfirewall.oc1..w1", compartment_id="c1", display_name="my-waf",
    ...
    result = build_flat_records(
        ...
        identity=_identity(), object_storage=_object_storage(),
        cloud_guard=_cloud_guard(), monitoring=_monitoring(),
        load_balancer=_load_balancer(), waf=_waf([web_app_firewall]), kms_vault=_kms_vault(),
        completed_at=COMPLETED_AT,
    )

    assert len(result.records) == 1
    record = result.records[0]
    assert record["id"] == "ocid1.webappfirewall.oc1..w1"
    assert record["evidenceType"] == "waf"
    assert record["loadBalancerId"] == "ocid1.loadbalancer.oc1..lb1"
    assert "status" not in record
    assert validate_record(record, load_flat_schema()).valid


def test_deleted_waf_is_excluded() -> None:
    web_app_firewall = _stamp(
        oci.waf.models.WebAppFirewallLoadBalancerSummary(
            id="ocid1.webappfirewall.oc1..deleted1", compartment_id="c1",
            backend_type="LOAD_BALANCER", lifecycle_state="DELETED",
        )
    )
    result = build_flat_records(
        decisions=DECISIONS, discovery=_discovery(), compute=_compute([]),
...
# -- KMS keys -- raw auto-rotation enabled + last-rotation timestamp --


def test_kms_key_reports_raw_rotation_facts() -> None:
    key = _stamp(
        oci.key_management.models.Key(
            id="ocid1.key.oc1..k1", compartment_id="c1", vault_id="ocid1.vault.oc1..v1",
            display_name="my-key", lifecycle_state="ENABLED", is_auto_rotation_enabled=True,
            auto_key_rotation_details=oci.key_management.models.AutoKeyRotationDetails(
                time_of_last_rotation=datetime.datetime(2026, 6, 1, tzinfo=datetime.UTC),
            ),
        )
    )
    result = build_flat_records(
        ...
        identity=_identity(), object_storage=_object_storage(),
        cloud_guard=_cloud_guard(), monitoring=_monitoring(),
        load_balancer=_load_balancer(), waf=_waf(), kms_vault=_kms_vault(keys=[key]),
        completed_at=COMPLETED_AT,
    )

    assert len(result.records) == 1
    record = result.records[0]
    assert record["id"] == "ocid1.key.oc1..k1"
    assert record["evidenceType"] == "kms_key"
    assert record["vaultId"] == "ocid1.vault.oc1..v1"
    assert record["autoRotationEnabled"] is True
    assert record["lastRotationAt"] == "2026-06-01T00:00:00Z"
    assert "status" not in record
    assert validate_record(record, load_flat_schema()).valid


def test_kms_key_without_rotation_details_reports_null_not_false() -> None:
    key = _stamp(
        oci.key_management.models.Key(
            id="ocid1.key.oc1..k2", compartment_id="c1", vault_id="ocid1.vault.oc1..v1",
            lifecycle_state="ENABLED", is_auto_rotation_enabled=False,
        )
    )
    result = build_flat_records(
        decisions=DECISIONS, discovery=_discovery(), compute=_compute([]),
    ...
    record = result.records[0]
    assert record["autoRotationEnabled"] is False
    assert record["lastRotationAt"] is None


def test_deleted_kms_key_is_excluded() -> None:
    key = _stamp(
        oci.key_management.models.Key(
            id="ocid1.key.oc1..deleted1", compartment_id="c1", vault_id="ocid1.vault.oc1..v1",
            lifecycle_state="DELETED", is_auto_rotation_enabled=False,
        )
    )
    result = build_flat_records(
        decisions=DECISIONS, discovery=_discovery(), compute=_compute([]),
